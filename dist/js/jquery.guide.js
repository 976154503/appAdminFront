...
/**
 * 利用了outline原理，页面仅支持一个元素的显示，不支持同时包含的多个元素显示；
 */
$.guide = function (options) {
	...
	*/
	...
	if (!window.localStorage || !options || !$.isArray(options) || localStorage[urlRoot] == '2') {
		...
		var elGuideOverlay = $('#elGuideOverlay');
		var elGuideShut = $('#elguideShut');
		...
		var goto = function (change) {
			// 创建层
			// var eachelGuideOverlay = '#guideOverlay' + change;
			// var elGuideOverlay = $(eachelGuideOverlay);
			// var eachelguideShut = '#guideShut' + change;
			// var elGuideShut = $(eachelguideShut);
			// var eachguideOverlap = '#guideOverlap' + change;
			// var elGuide = $(eachguideOverlap);
			...
			data = $.extend({}, defaults, options[start]);
			...
			console.log(elTrigger.offset().left);
			// 定位
			elGuide.css({
